...
export class PolicyService {
   ...
   */
  static async getPolicy(id: number, include?: string[]): Promise<{ data: Policy }> {
    const params = new URLSearchParams();
  ...
  static async updatePolicy(
    ...
    const response: AxiosResponse<ApiResponse<Policy>> = await api.put(
      `/policies/${id}`,
      data,
    );
  ...
  static async getInstallments(policyId: number): Promise<Installment[]> {
    ...
    return installments.map((installment: any) => {
      const isCredit = installment.transaction_reference && 
   ...
   */
  static async getPolicyInstallments(policyId: number): Promise<{data: Installment[]}> {
  ...
  static async getInstallmentDashboardStats(): Promise<InstallmentDashboardStats> {
    const response: AxiosResponse<ApiResponse<InstallmentDashboardStats>> = await api.get(
      '/installments/dashboard-stats',
    );
   ...
   */
  static async getOverdueInstallments(agentId?: number, daysOverdue?: number): Promise<{
    data: EnhancedInstallment[];
    summary: {
  ...
  static async getActiveGracePeriods(): Promise<GracePeriod[]> {
    const response: AxiosResponse<ApiResponse<{data: GracePeriod[]}>> = await api.get(
      "/grace-periods/active"
    );

    if (response.data.status !== 'success') {
      throw new Error(response.data.message || 'Failed to fetch active grace periods');
    }
  ...
  static async sendGracePeriodReminder(gracePeriodId: number): Promise<void> {
    try {
      ...
      if (response.data.status === 'error' || (response.data.success !== undefined && !response.data.success)) {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (status === 400) {
          throw new Error(responseData?.message || 'Invalid request. The grace period may not be active.');
        } else if (status === 401) {
   ...
   */
  static async getAuditLogs(policyId: number): Promise<Array<Record<string, unknown>>> {
    const response: AxiosResponse<ApiResponse<Array<Record<string, unknown>>>> = await api.get(
      `/policies/${policyId}/audits`,
    );
  ...
  /**
   * Simple update for family member assignment only
   */
  static async updatePolicyFamilyMember(
    id: number,
    familyMemberId: number | null,
  ): Promise<Policy> {
    // First get the current policy data to include required fields
    ...
    const response: AxiosResponse<ApiResponse<Policy>> = await api.put(
      ...
      { 
        ...
        // Include minimal required fields from current policy
        plan_id: currentPolicy.data.plan_id,
        start_date: currentPolicy.data.start_date,
        premium_amount: currentPolicy.data.premium_amount,
        premium_frequency: currentPolicy.data.premium_frequency || 'monthly'
      },
    );
