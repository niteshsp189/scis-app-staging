...
export const OfficeLocationValidatedInput = React.forwardRef<HTMLInputElement | HTMLTextAreaElement, ValidatedInputProps>(({
  id,
  label,
  value,
  onChange,
  ...
  const combinedRef = (ref as React.RefObject<HTMLInputElement | HTMLTextAreaElement>) || (asTextarea ? textareaRef : inputRef);
  ...
  const getMessageType = () => {
    ...
    if (isNearLimit || isFocused) return "info";
    return "hint";
  };
  ...
  const shouldShowValidation = error || isAtLimit || isNearLimit || (validationHint && isFocused);

  // Show validation hint on focus when no error
  const shouldShowHint = !error && validationHint && isFocused;

  return (
    // reduced vertical spacing so validation label sits closer to the input
    <div className="space-y-2 group">
      ...
      {/*  reserved height so message appears closer */}
        ...
        {validationMessage && (
          <div
            className={cn(
              ...
              messageType === "warning" && "text-orange-600 bg-orange-50 border border-orange-200",
...
export const OfficeLocationValidatedSelect: React.FC<ValidatedSelectProps> = ({
  id,
  label,
  value,
  onChange,
