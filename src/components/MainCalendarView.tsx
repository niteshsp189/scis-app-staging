...
export const MainCalendarView = forwardRef<{ refreshAppointments: () => Promise<void> }>((props, ref) => {
  const isMobile = useIsMobile();
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [view, setView] = useState<CalendarView>("month");
  const [intervalType, setIntervalType] = useState<"hourly" | "5min" | "15min">("hourly");
  ...
  const [filterOfficeLocation, setFilterOfficeLocation] = useState<string>("all");
  ...
  useEffect(() => {
    ...
    loadUsers();
    loadOfficeLocations();
  }, [currentDate, view, filterEmployee, filterOfficeLocation, filterAppointmentType, filterStatus, filterCreatedBy, filterDateFrom, filterDateTo]);
      ...
      try {
        ...
        if (!isNaN(selectedDate.getTime())) {
          if (view === "month") {
            ...
            if (weekStart.getTime() !== startOfWeek(currentDate, { weekStartsOn: 0 }).getTime()) {
      ...
      const appointmentEvents = appointments
        .filter((apt) => {
          ...
          try {
            ...
            console.warn("Invalid date format for appointment:", apt.start_datetime);
            return false;
          }
        })
        .map((apt) => {
          try {
            ...
            const appointmentTypeLabel = appointmentService.getAppointmentTypeLabel(apt.appointment_type);
            const assignedUserName = apt.assigned_user ? `${apt.assigned_user.first_name} ${apt.assigned_user.last_name}` : '';
            ...
            return {
              ...
              customer: apt.customer ? `${apt.customer.first_name || ''} ${apt.customer.last_name || ''}`.trim() : '',
      ...
      const leaveEvents = leaveRequests
        .filter((leave) => {
          ...
          try {
            ...
            const leaveEndDate = leave.end_date ? new Date(leave.end_date) : leaveStartDate;
            ...
            if (!leave.end_date || leave.start_date === leave.end_date) {
              ...
              return isSameDay(date, leaveStartDate) || 
                     isSameDay(date, leaveEndDate) || 
                     (date > leaveStartDate && date < leaveEndDate);
            }
          } catch (error) {
            console.warn("Invalid date format for leave request:", leave.start_date);
            return false;
          }
        })
        .map((leave) => {
      ...
      const holidayEvents = holidays
        .filter((holiday) => {
          ...
          try {
            ...
            console.warn("Invalid date format for holiday:", holiday.date, error);
            return false;
          }
        })
        .map((holiday) => {
  ...
  const hasFullDayLeaveForEmployee = (date: Date, employeeId?: string): boolean => {
    ...
    return leaveRequests.some(leave => {
      ...
      const leaveEndStr = leave.end_date ? leave.end_date.split('T')[0] : leaveStartStr;
      const checkDateStr = format(date, 'yyyy-MM-dd');

      // Check if the date falls within the leave period (inclusive)
      const isWithinLeavePeriod = checkDateStr >= leaveStartStr && checkDateStr <= leaveEndStr;
  ...
  const hasLeaveRequestsForEmployee = (date: Date, employeeId?: string) => {
    ...
    return leaveRequests.some(leave => {
      ...
      const isWithinLeavePeriod = checkDate >= leaveStart && checkDate <= leaveEnd;
  ...
  const hasLeaveRequestsForEmployeeAtTime = (date: Date, hour: number, employeeId?: string) => {
    if (!employeeId || employeeId === 'all') return false;

    return leaveRequests.some(leave => {
      const leaveStart = new Date(leave.start_date + 'T00:00:00');
      const leaveEnd = leave.end_date ? new Date(leave.end_date + 'T00:00:00') : leaveStart;
      const checkDate = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0);

      // Check if the date falls within the leave period (inclusive)
      const isWithinLeavePeriod = checkDate.getTime() >= leaveStart.getTime() && checkDate.getTime() <= leaveEnd.getTime();
  ...
  const getLeaveRequestForEmployeeAtTime = (date: Date, hour: number, employeeId?: string): LeaveRequest | null => {
  ...
  const getLeaveRequestsForEmployeeAtTime = (date: Date, hour: number, employeeId?: string): LeaveRequest[] => {
  ...
  const renderEvents = (events: CalendarEvent[], hour: number, isWeekOrDayView: boolean = false) => {
    ...
    return (
      <div className={`flex ${isWeekOrDayView ? 'flex-col space-y-0.5' : 'space-x-1'} h-full ${isWeekOrDayView ? 'w-full max-w-full' : 'overflow-x-auto'}`}>
        {events.map((event) => {
          ...
          let colorClasses = { background: 'bg-gray-600', text: 'text-white', hover: 'hover:opacity-80' };
          ...
          if (isHoliday) {
            ...
            const leaveType = event.title.includes('Partial Day') ? 'partial_day' : 'full_day';
            colorClasses = getLeaveColorClasses(leaveType);
          } else if (event.status === 'cancelled') {
            colorClasses = { background: 'bg-red-600', text: 'text-white', hover: 'hover:opacity-80', border: 'border-red-700' };
          } else {
          ...
          return (
            ...
            >
              {isHoliday && (
                <div className={`text-[10px] font-bold mb-0.5 px-1 py-0.5 rounded text-center ${colorClasses.badgeBg || 'bg-opacity-80 bg-black'}`}>
                  HOLIDAY
                </div>
              )}
              {isLeave && (
              ...
              {!isHoliday && !isLeave && <div className="text-[9px] opacity-90 truncate">{event.customer}</div>}
              {isHoliday && event.customer && <div className="text-[9px] opacity-90 truncate">{event.customer}</div>}
              {isLeave && event.customer && (
                <div className="text-[9px] opacity-90 truncate">{event.customer}</div>
              )}
            </div>
          );
        })}
  ...
  const renderMonthView = () => {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        const hasHolidays = dayEvents.some(event => event.type.startsWith('holiday-'));
        const hasFullDayLeave = hasFullDayLeaveForEmployee(currentDay, filterEmployee);
        ...
        days.push(
          <div
            ...
            onClick={() => !(hasHolidays || isDisabledForLeave) && handleDateClick(currentDay)}
          >
            ...
            <div className="space-y-0.5 md:space-y-1 max-h-12 md:max-h-20 overflow-y-auto">
              {dayEvents.slice(0, isMobile ? 2 : dayEvents.length).map((event) => {
                const isHoliday = event.type.startsWith('holiday-');
                const isLeave = event.type.startsWith('leave-');
                ...
                return (
                  <div
                    ...
                    title={isHoliday ? `${event.title}${event.customer ? ' - ' + event.customer : ''}` : 
                           isLeave ? `${event.title} - ${event.customer}` : 
                  ...
                  >
                    {isHoliday && (
                      <div className={`text-xs font-bold mb-1 px-1 py-0.5 rounded text-center ${colorClasses.badgeBg || 'bg-opacity-80 bg-black'}`}>
                        HOLIDAY
                      </div>
                    )}
                    {isLeave && (
      ...
      rows.push(
        <div key={day.toString()} className="grid grid-cols-7 divide-x-2 divide-gray-300">
          {days}
        </div>
      );
      days = [];
      ...
      <div className="bg-white rounded-lg border-2 border-gray-300 shadow-sm">
        <div className="grid grid-cols-7 border-b-2 border-gray-300 bg-gray-50">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div key={day} className="p-2 md:p-4 text-center font-medium text-xs md:text-sm text-gray-700 border-r-2 border-gray-300 last:border-r-0">
              {day}
            </div>
          ))}
        </div>
  ...
  const renderWeekView = () => {
    ...
    const getTimeSlots = () => {
      if (intervalType === "5min") {
        return Array.from({ length: 24 * 12 }, (_, i) => ({ hour: Math.floor(i / 12), minute: (i % 12) * 5 }));
      } else if (intervalType === "15min") {
        return Array.from({ length: 24 * 4 }, (_, i) => ({ hour: Math.floor(i / 4), minute: (i % 4) * 15 }));
      } else {
    ...
    return (
      <div className="bg-white rounded-lg border overflow-x-auto">
        <div className="grid gap-0 border-b min-w-[640px]" style={{ gridTemplateColumns: `100px repeat(7, 1fr)` }}>
          <div className="p-2 md:p-4 border-r"></div>
          {days.map((day) => {
            const isToday = isSameDay(day, new Date());
            const dayEvents = getEventsForDate(day);
            const hasHolidays = dayEvents.some(event => event.type.startsWith('holiday-'));
            const hasFullDayLeave = hasFullDayLeaveForEmployee(day, filterEmployee);
            // Only disable click when specific employee is filtered and has leave
            const isDisabledForLeave = filterEmployee !== 'all' && hasFullDayLeave;
            return (
              <div
                ...
                onClick={() => !(hasHolidays || isDisabledForLeave) && handleDateClick(day)}
              >
        ...
        <div className="grid gap-0 min-w-[640px]" style={{ gridTemplateColumns: `100px repeat(7, 1fr)` }}>
          <div className={`border-r ${isMinuteView ? 'space-y-0' : 'space-y-4'}`}>
              ...
              return (
                ...
                >
                  {isHourMark ? (
                    ...
                    <span className="text-[7px]">{String(slot.minute).padStart(2, '0')}</span>
                  )}
                </div>
              );
            })}
          ...
          {days.map((day) => (
            <div key={day.toString()} className={`border-l min-w-0 ${isMinuteView ? 'space-y-0' : 'space-y-4'}`}>
              {timeSlots.map((slot, idx) => {
                const leaveRequestsAtTime = getLeaveRequestsForEmployeeAtTime(day, slot.hour, filterEmployee);
                const hasFullDayLeave = hasFullDayLeaveForEmployee(day, filterEmployee);
                ...
                const leavesToShow = leaveRequestsAtTime.filter(leave => (
                ...
                const slotEvents = getEventsForDate(day)
                  .filter(event => {
                    ...
                    if (leaveRequestsAtTime.length > 0 && event.type.startsWith('leave-')) {
                      return false;
                    }
                    ...
                    if (intervalType === "5min") {
                      ...
                      return eventHour === slot.hour && eventMinute >= slot.minute && eventMinute < slot.minute + 5;
                    } else if (intervalType === "15min") {
                      const eventHour = event.start.getUTCHours();
                      const eventMinute = event.start.getUTCMinutes();
                ...
                const dayHasHolidays = getEventsForDate(day).some(event => event.type.startsWith('holiday-'));
                const hasLeaveRequests = hasLeaveRequestsForEmployeeAtTime(day, slot.hour, filterEmployee);
                const hasAppointments = slotEvents.length > 0 && slotEvents.some(e => !e.type.startsWith('leave-') && !e.type.startsWith('holiday-'));
                const shouldShowLeaveDetails = shouldShowLeaveInfo || hasLeaveRequests;
                const shouldShowAppointmentDetails = hasAppointments;
                // Only disable click when specific employee is filtered and has leave
                const isDisabledForLeave = filterEmployee !== 'all' && (hasFullDayLeave || hasLeaveRequests || leaveRequestsAtTime.length > 0);
                const cellHeight = isMinuteView ? 'h-16 md:h-20' : 'h-12 md:h-16';
                ...
                return (
                  <div
                    ...
                    className={`${cellHeight} border-b p-0.5 md:p-1 relative text-[10px] md:text-xs ${
                      ...
                      shouldShowLeaveInfo || (slot.hour === 0 && slotEvents.length > 0)
                        ? 'flex items-stretch overflow-hidden'
                        : 'flex items-center justify-center overflow-hidden'
                    } group`}
                    onClick={() => !(dayHasHolidays || isDisabledForLeave) && handleDateClick(day, slot.hour, slot.minute)}
                    title={displayTime12hr}
                  >
                    {isMinuteView && !slotEvents.some(e => !e.type.startsWith('leave-') && !e.type.startsWith('holiday-')) && !shouldShowLeaveInfo && (
                    ...
                    {shouldShowLeaveInfo && slot.hour === 0 && slotEvents.length > 0 ? (
                      ...
                      <div className="w-full max-w-full h-full flex flex-col space-y-0.5 overflow-y-auto overflow-x-hidden p-0.5">
                        ...
                        {leavesToShow.map((leaveRequest) => {
                          const leaveColors = leaveRequest?.leave_type === 'partial_day' 
                            ? getLeaveColorClasses('partial_day')
                            : getLeaveColorClasses('full_day');
                          return (
                            <div key={`leave-${leaveRequest.id}`} className={`w-full max-w-full rounded text-xs flex flex-col ${leaveColors.background} ${leaveColors.text} p-1`}>
                              <div className={`text-[10px] font-bold mb-0.5 px-1 py-0.5 rounded text-center flex-shrink-0 ${leaveColors.badgeBg || 'bg-opacity-80 bg-black'}`}>
                                LEAVE
                              </div>
                              ...
                              <div className="text-[8px] opacity-90 truncate flex-shrink-0">
                                Mgr: {leaveRequest?.manager?.name ||
                                  `${leaveRequest?.manager?.first_name || ''} ${leaveRequest?.manager?.last_name || ''}`.trim()}
                              </div>
                              <div className="text-[8px] opacity-90 flex-shrink-0">Full Day</div>
                            </div>
                          );
                        })}
                      </div>
                      ...
                      <div className="w-full h-full flex flex-col space-y-0.5 overflow-y-auto overflow-x-hidden p-0.5">
                        {leavesToShow.map((leaveRequest) => {
                          const leaveColors = leaveRequest?.leave_type === 'partial_day' 
                            ...
                            : getLeaveColorClasses('full_day');
                          return (
                            <div key={`leave-${leaveRequest.id}`} className={`w-full rounded text-xs flex flex-col overflow-hidden ${leaveColors.background} ${leaveColors.text}`}>
                              <div className="px-1 py-1 flex flex-col h-full overflow-y-auto">
                                <div className={`text-[10px] font-bold mb-1 px-1 py-0.5 rounded text-center flex-shrink-0 ${leaveColors.badgeBg || 'bg-opacity-80 bg-black'}`}>
                                  LEAVE
                                </div>
                              ...
                              {leaveRequest?.leave_type === 'partial_day' && leaveRequest?.start_time && leaveRequest?.end_time ? (
                                <div className="text-[9px] opacity-95 mb-0.5 flex-shrink-0">
                                  Partial Day ({formatTime12Hour(leaveRequest.start_time)} - {formatTime12Hour(leaveRequest.end_time)})
                                </div>
                              ) : (
                                <div className="text-[9px] opacity-95 mb-0.5 flex-shrink-0">Full Day</div>
                              )}
                              {leaveRequest?.reason && (
                                <div className="text-[8px] opacity-90 break-words" title={leaveRequest.reason}>
                                  {leaveRequest.reason}
                                </div>
                              )}
                            </div>
    ...
    return (
      <div className="bg-white rounded-lg border">
        <div className="p-3 md:p-4 border-b text-center">
          <div className="text-base md:text-lg font-bold">{format(currentDate, "EEEE")}</div>
          <div className="text-xl md:text-2xl font-bold text-blue-600">{format(currentDate, "dd")}</div>
          <div className="text-xs md:text-sm text-gray-500">{format(currentDate, "MMMM yyyy")}</div>
        </div>
        
        <div className="grid gap-0" style={{ gridTemplateColumns: isMinuteView ? '80px 1fr' : '60px 1fr' }}>
          <div className={`border-r ${isMinuteView ? 'space-y-0' : 'space-y-4'}`}>
            {timeSlots.map((slot, idx) => {
              const cellHeight = isMinuteView ? 'h-16 md:h-20' : 'h-12 md:h-16';
              const isHourMark = slot.minute === 0;
          ...
          <div className={`border-l ${isMinuteView ? 'space-y-0' : 'space-y-4'}`}>
            {timeSlots.map((slot, idx) => {
              const leaveRequestsAtTime = getLeaveRequestsForEmployeeAtTime(currentDate, slot.hour, filterEmployee);
              const hasFullDayLeave = hasFullDayLeaveForEmployee(currentDate, filterEmployee);
              ...
              // Check if there are holidays on this day
              const dayHasHolidays = dayEvents.some(event => event.type.startsWith('holiday-'));
              const hasLeaveRequests = hasLeaveRequestsForEmployeeAtTime(currentDate, slot.hour, filterEmployee);
              const hasAppointments = slotEvents.length > 0 && slotEvents.some(e => !e.type.startsWith('leave-') && !e.type.startsWith('holiday-'));
              const shouldShowLeaveDetails = shouldShowLeaveInfo || hasLeaveRequests;
              const shouldShowAppointmentDetails = hasAppointments;
              // Only disable click when specific employee is filtered and has leave
              ...
              return (
                <div
                  ...
                  onClick={() => !(dayHasHolidays || isDisabledForLeave) && handleDateClick(currentDate, slot.hour, slot.minute)}
                  title={displayTime12hr}
                >
                  {shouldShowLeaveInfo && slot.hour === 0 && slotEvents.length > 0 ? (
                    // Show both holidays/other events AND full-day leaves in 12 AM slot
  ...
  return (
    <div className="space-y-4">
        ...
        {isFiltersExpanded && (
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              {/* Assignee Filter */}
              <div>
                <label className="text-sm font-medium mb-2 block">Filter by Assignee</label>
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">Filter by Created By</label>
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">Filter by Office Location</label>
                <Select value={filterOfficeLocation} onValueChange={setFilterOfficeLocation}>
                  <SelectTrigger>
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">Filter by Appointment Type</label>
      ...
      <div className="flex flex-col space-y-3 md:space-y-0 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            <Button variant="outline" size={isMobile ? "sm" : "default"} onClick={navigatePrevious}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button variant="outline" size={isMobile ? "sm" : "default"} onClick={goToToday}>
              {isMobile ? "Today" : "Today"}
            </Button>
            <Button variant="outline" size={isMobile ? "sm" : "default"} onClick={navigateNext}>
      ...
      <ScheduleMeetingWithCustomerDialog
        ...
        defaultDate={selectedDate || new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate())}
      />
